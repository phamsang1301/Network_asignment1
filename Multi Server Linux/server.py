...
def changeTcpPort(udpPort, clientIP, id):
    ...
    newThread = Thread(target=dataConnection2, args=(
        newServer, newDataSocket2, newDataPort, id))
    newThread.start()
...
def dataConnection2(server, dataSocket2, dataPort, id):
...
def registerConnection(registerSocket, id):
    ...
    #wrong header
        ...
        displayClientIP = message.split()[4]
        ...
        elif (message2.split()[0] != 'DATAPORT-REPLY'):
            datasocketStatusMessage = '302 Wrong Header'
            registerSocket.send(datasocketStatusMessage.encode())
        elif (message2.split()[2] != 'accept' and message2.split()[2] != 'deni'):
        ...
        if (result == 'accept'):
            ...
            changeTcpPortButton = tk.Button(text='Change', command=partial(changeTcpPort, udpPort, clientIP, id))
...
window = tk.Tk()
window.title("Computer Resource Management")
