...
# cpuTempThread = Thread(target=cputempFunc, args=())
# cpuTempThread.start()
...
def getInfo():
    ...
    cDisk = psutil.disk_usage('C:/')
    dDisk = psutil.disk_usage('D:/')
    total = round(((cDisk.total + dDisk.total) / pow(2, 30)), 1)
    used = round(((cDisk.used + dDisk.used) / pow(2, 30)), 1)

    available = round(((cDisk.free + dDisk.free) / pow(2, 30)), 1)
    ...
    infos = 'COMPUTER-INFO\n\nCPU: ' +str(cpuTemp) + ' \N{Degree Celsius}'+ '\n\n'
...
def startClient():
    global udpPort, udpPortEntry, setServer

    if (udpPortEntry.get() == '' or nameInput.get() ==  '' or ipInput.get() == '' or setServer  == False):
